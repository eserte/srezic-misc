...
of plugins and additions. A list of DateTime-related modules may be
found at L<http://datetime.perl.org/?Modules>.
...
L<Storable> has the function B<dclone()>.
...
See also below for comparing within test scripts (L<Test::Deep>).
...
database is costly, but reading is very fast. A pure perl variant is
L<CDB_Perl>.
...
L<Devel::NYTProf> is a fork of Devel::FastProf, which includes extra
reporting. See its Pod for a list of subroutine- and statement-level
profilers.
...
compares against the other available ssh implementations.
...
benchmark:
L<http://search.cpan.org/~rhandom/Template-Alloy-1.009/samples/benchmark/bench_various_templaters.pl>.
The examples directory of L<HTML::Template::Compiled> also includes a
benchmark script, but without numbers in it.
...
is used in String::Approx, for exampl) between two strings.
