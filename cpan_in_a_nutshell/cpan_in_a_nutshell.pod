...
The first YAML module is the pure perl implementation L<YAML>. It is
somewhat slower than the competitors written in C/XS.
...
L<CPAN::Meta::YAML> handles a subset of YAML, just enough to parse
CPAN Meta files. It has the advantage for being in the perl core since
5.13.9.

=head3 JSON/Javascript

JSON is a human readable format which is based on javascript syntax.
It's not so complex as YAML (especially is unable to serialize many
perl types like blessed objects, regexps, code references,
self-referential data...), but has the other advantages of YAML. A
pure perl implementation is L<JSON>.

L<JSON::XS> is another C based parser/generator for JSON. It seems to
be the only one which handles Unicode correctly.
...
L<CBOR::XS>: CBOR (Concise Binary Object Representation) is a binary
serialization which follows the JSON data model, and adds some
additional capabilities, like serialization of perl objects. According
to the manpage, it is faster than L<JSON::XS> and L<Storable>.
...
(de)serialize code references. Storable can (de)serialize using
strings and filehandles; the latter helps in reducing the memory
footprint when (de)serializing large data structures.
...
L<Sereal::Decoder> and L<Sereal::Encoder> are fast and
space-efficient. More objectives are described in
L<https://github.com/Sereal/Sereal/blob/master/README.pod>.
...
Google's protocol buffers format is implemented by
...
L<JSON::Schema> is a module handling a schema language originally
written for JSON data, but may also be used for simple Perl data
structures.

L<Sah> is another schema for data structures.
...
To create a "flat" representation from a nested hash structure
L<Hash::Flatten> may be used. Its Pod documentation mentions more
similar modules: L<CGI::Expand> and L<Tie::MultiDim>. L<Hash::Fold> is
similar to L<Hash::Flatten>, but is built using Moose.
...
to modify the elements in-place. Related modules are: L<Data::Dmap>,
L<Data::Walk> (which implements a L<File::Find>-like interface),
L<Data::Transformer>, L<Data::Traverse>, L<Data::Visitor>.
...
date/time functions with a functional (using somewhat non-perlish
naming) und object-oriented interfaces. It's written in C, but the
functionality is also available in the pure perl L<Date::Calc::PP>
module (and historically there was another pure perl replacement
L<Date::Pcalc>).

L<Time::Moment> is a C based module which is faster than L<DateTime>.
It has functionality for constructing a date object from epoch,
parameters, and iso8601-like strings; and for arithmetic, comparisons
and strftime-like formatting.
...
L<accessors>: simple syntax, has also some variants. A possible
downside is that the internal hash members are preceded with a dash.
This makes serialized objects (e.g. as YAML) not-so-nice to read and
write.
...
(according to the documentation) much faster than Moose. Other even
tinier versions of Moose are L<Moo> and L<Mo>.

=head3 Inside-out technique

L<Class::InsideOut::Manual::About> explains how L<Class::InsideOut>
works and has a list of other modules implementing the inside-out
technique.
...
L<Sort::Versions>, L<CPAN::Version>, L<version>, L<Version::Compare>
may be used to compare and sort version numbers.
...
scripts (L<Test::Differences>, L<Test::Deep>,
L<Test::More/is_deeply>).

To compare files, use the core module L<File::Compare>.

=head2 Merging

The following modules deal with merging data structures:
L<Hash::Merge>, L<Hash::Merge::Simple>, L<Data::Merger>,
L<Data::ModeMerge>.

=head2 Caching

See L<CHI>, L<Cache>, L<Cache::Cache>, and others.
...
L<SQL::Abstract> is inspired by L<DBIx::Abstract>, but does only the
SQL generation part. L<SQL::Abstract::Limit> adds a portable LIMIT
emulation to that module.

The L<SQL::Maker> documentation claims that it's more extensible than
L<SQL::Abstract>. L<SQL::Maker> is inspired by L<DBIx::Skinny>.

With L<SQL::Interpolate> one still writes SQL statements, but perl
variables may be magically interpolated into them.

More approaches: L<SQL::DB::Schema>, L<SQL::Query> and many more at
CPAN.
...
=head3 Module prerequisites

L<Module::PrintUsed> prints the list of used modules at the end of a
script run.

L<Perl::PrereqScanner> gets a list of used modules by scanning perl
files.

Another helper module: L<Module::Extract::Use>.
...
L<Git::PurePerl> is a pure perl interface to Git repositories.
...
L<Regex::PreSuf>. L<Regexp::Debugger> shows nicely how the regexp
engine is working, step by step.
...
L<Pod::Simple>: a robust Pod parser. It's part of the perl
distribution since 5.10.0.

L<Pod::POM>: converts Pod documents into an object module, which can
in turn be translated into different markups like HTML, text, or again
Pod. Unfortunately the Pod parser has a couple of issues (see RT bug
tickets).

L<Tk::Pod>: a Tk Pod viewer widget.
...
L<PerlIO::gzip> and L<PerlIO::via::gzip> both provide transparent gzip
compression and decompression while reading from/writing to file
handles.

L<IO::Compress::Base> and L<IO::Uncompress::Base> are base classes for
a number of modules dealing with compression, including gzip, zip, and
bzip2. This modules are in core since perl 5.9.4.
...
like the MIME type recognition of Apache. A similar module with a
simpler interface is L<Media::Type::Simple>.
...
Debian or as a FreeBSD port). And another one is L<File::Rename>.
...
Other modules here are: L<File::Next>, L<Path::Class::Rule>,
L<Path::Iterator::Rule>, L<File::Find::Rule>, L<File::Find::Object>,
L<File::Find::Iterator>. These are covered in this rjbs article:
L<http://rjbs.manxome.org/rubric/entry/1981>. rjbs also mentions the
following modules which do not do lazy iteration, so these are
probably slow on large trees: L<File::Find::Declare>,
L<File::Find::Match>, L<File::Find::Node>. And finally there's
L<File::Find::Wanted>.

L<Directory::Iterator::XS> is a recursive directory lister,
implemented as XS.

L<File::Zglob> provides globbing like the core L<File::Glob>, but adds
zsh's "**" pattern, which can be used to recursively descend into
subdirectories.

L<File::Locate> uses the locate/slocate database which is usually
updated every night on Unix boxes.

A speed research: L<http://rjbs.manxome.org/rubric/entry/1981>.

=head3 Changing the directory

For a simple change of the current working directory, use the perl
builting L<chdir|perlfunc/chdir>.

L<File::pushd> allows a temporary change of the current working
directory, which is automatically reverted one the created guard
object goes out of scope.

L<File::chdir> can do the same, but it uses a localized variable
C<$CWD> instead.

L<File::Tools> provides a number of commonly found file-handling Unix
tools as Perl functions; amongst them also C<pushd> and C<popd>. Note
that this version is not capable of automatically revert the
directory.

L<Cwd::Guard> works similar as L<File::pushd>. It's using fchdir under
the hood, so should be safe against directory renames while the guard
is in effect.

L<File::cd> is another module for temporary directory changes with a
different interface: here a callback function is supplied for the code
which should run within the changed directory.
...
Perl already has core support for flock(). But there are modules which
are wrappers around it, especially for creating lock files, or
implement locking a different way.

L<File::Flock> is a wrapper around flock(). It knows about blocking
and non-blocking locks and has a convenient OO interface which removes
the lock file if the object goes out of scope. The distribution
includes also File::Flock::Subprocess which handles locks in
subprocesses for operating system which do not do this normally (e.g.
Solaris), but this makes the dependency chain a little bit longer.

L<File::Flock::Tiny> is another, probably smaller wrapper around
flock().

L<File::Basic::Flock> is similar like F<File::Flock>, but the user has
to create the lock file himself.

L<File::FcntlLock> uses fcntl() instead of flock() for locking. One
advantage is that fcntl() can do range locking. Disadvantage is that
this method is not available everywhere.

The locking algorithm of L<LockFile::Simple> does not use any system
calls like flock() or lockf(). It has even support for locking over
NFS. It is reasonable safe from race conditions, but not absolutely.

The following modules have also some support for locking:
L<Paranoid::Lockfile> (but check the RT queue here!).

L<File::Lockfile> is not recommended, as race conditions are easily
possible (checked with at least v1.0.5).

L<Sys::RunAlone> is a convenient wrapper around C<flock()> for scripts
which should run only once at a time, optionally with a retry option
and being silent or loud, if there's another script running.
...
is also truetype/freetype font support.
...
like libpng and libjpeg installed on your system.
...
L<Cairo> is a 2d vector graphics library and able to create png files,
...
[XXX: What are the differences between both?]

L<SVG> provides an API to create SVG graphics.

L<Cairo> is a 2d vector graphics library and able to create SVG
graphics, amongst other formats.
...
L<Chart::Clicker> is based on the L<Cairo> graphics library and many
...
L<Exporter::Lite>, L<Exporter::Easy>, L<Exporter::Simple> (this one's
broken for perl 5.12 and newer and seems to be not fixed anymore),
...
Last, with L<Inline::C> C code may be embedded in Perl code.
...
The classic module is L<CPAN>, which comes with core perl. An
alternative is L<CPANPLUS>, which was temporarily part of the perl
core (but not anymore since 5.16.0). L<App::cpanminus> (aka L<cpanm>)
is another alternative.

L<CPAN::Mini> is a module for creating a minimal mirror of the CPAN
repository.
...
existance of a C<make> tool on the system. For a pure-perl
build system try L<Module::Build>.

L<Module::Install> is a build description system which provides a nice
DSL and is extensible using plugins. It's self-contained;
distributions are expected to ship C<Module::Install> and all
dependencies. Under the hood it is using L<ExtUtils::MakeMaker> and
relies on a C<make> tool.

L<Dist::Zilla> is a distribution builder.
...
commands (XXX better description!).
...
L<Slay::Makefile> provides a make variant called C<slaymake> which can
run both shell commands and perl code.

L<Parallel::Depend> is a dependency system which has a focus on
parallel execution. It provides also a dependency system which looks
remotely like a makefile. It's able to run both shell comands and perl
code.

=head2 Parsers

L<Marpa::XS> is a parser for grammars which can be written in BNF.

See L<http://egparser.sourceforge.net/> about a comparison between
L<Parse::RecDescent>, L<Parse::Yapp>, and Perl-byacc.
...
The documentation page of L<Log::Fast>, a logger written for speed,
have a speed comparison against other logging modules at:
L<http://search.cpan.org/dist/Log-Fast/lib/Log/Fast.pm#DESCRIPTION>.
...
L<Rex> stands for remote execution and is a collection of modules to
help for system administrating remote servers.

=head2 Geo modules

=head3 Distance calculation

The core module L<Math::Trig> has a C<<great_circle_distance>> function.

L<Geo::Distance> offers a couple of different algorithms for distance
calculation. L<Geo::Distance::XS> is a faster XS implementation of
this module.

L<GIS::Distance> also offers different algorithms for distance
calculation. The returned values are objects here.
L<GIS::Distance::Fast> is a faster XS implementation of this module.
...
=head4 Working modules

L<Yahoo::Weather> does not need an API key, seems to still work, and
has data for international locations. It provides current weather
conditions, forecast, sunrise/sunset data.

L<Weather::Underground> does not need an API key, seems to still work,
and has data for international locations. It provides current weather
conditions, no forecast, sunrise/sunset and moonrise/moonset data.

L<Geo::METAR> parses METAR weather data. It does not do fetching at
all. A sample source for METAR data is
L<http://weather.noaa.gov/cgi-bin/mgetmetar.pl?cccc=I<$ICAO>> (put in
an icao airport code).

=head4 Non-working modules

L<Weather::Com>: the Pod says "Note that weather.com is retiring the
XML Data Feed."

L<Weather::Google> does not work anymore, because Google ceased the
iGoogle weather API. See
L<https://rt.cpan.org/Ticket/Display.html?id=79260>.

L<Geo::Weather> is US centric and does not seem to work anyway. See
also L<https://rt.cpan.org/Ticket/Display.html?id=1820>.

L<http://search.cpan.org/perldoc?weather> looks like a convenient
script, but unfortunately the location data is stored in the script
itself, and there does not seem to be a way how to change this from
cmdline.

=head4 Unknown status
...
=item * L<WebService::Livedoor::Weather>

=item * L<Weather::NOAA::GFS>

=item * L<Weather::Airport>

=item * L<Geo::WeatherNWS>

=item * L<Geo::WeatherNOAA>

=item * L<WWW::Weather::Yahoo>

Test is failing, so probably does not work anymore

=item * L<WWW::Wunderground::API>

=item * L<Weather::YR>

Maybe the API is no longer active? See L<https://rt.cpan.org/Ticket/Display.html?id=82048>.

=item * L<Weather::NWS::NDFDgen>

Module needs probably a URL update, see L<https://rt.cpan.org/Ticket/Display.html?id=71371>.
...
L<Net::SSH2> is a wrapper built around libssh2.
...
with a number of methods to access an XML document: using DOM
functions with XPath support, or using a pull parser (SAX), or a push
parser (L<XML::LibXML::Reader>).
...
L<XML::LibXML::Document> has also canonicalisation methods
(toStringC14N...).
...
L<http://www.perl.com/pub/2001/08/21/templating.html>. Here's a
benchmark:
...
including Template-Toolkit and HTML::Template. Unfortunately the XS
version, L<Template::Alloy::XS> does not work anymore for a long time.
...
L<PDF::Create> is a pure-perl module and works fine for generating pdf
documents, including ones with simple vector graphics.

L<PDF::API2> is big and complete. But it seems to be slower than
PDF::Create.

In L<CAM::PDF>'s SEE ALSO Pod section there's a comparison between a
couple of PDF-related modules.

L<Cairo> is a 2d vector graphics library and able to create PDF files,
amongst other formats.
...
More CPAN modules: L<Text::Brew>, L<Text::Fuzzy>, L<Text::Compare>,
L<Text::Dice>, L<Text::Levenshtein::XS>,
L<Text::Levenshtein::Damerau::PP>, L<Text::Levenshtein::Damerau::XS>,
L<Text::WagnerFischer>. See also
L<https://metacpan.org/pod/distribution/Text-Dice/ex/benchmark.pl> for
benchmark results.
...
also faster. These are L<Furl>, L<HTTP::Lite> and L<HTTP::Client>
(where the latter needs C<HTTP::Lite> as a dependency and the version
1.51 seems to be broken). L<HTTP::MHTTP> is also a very low-level
implementation, which is probably faster than L<LWP>, but for example
lacks out-of-the-box support for name-based virtual hosts.

L<HTTP::GHTTP> needs a non-standard library to be installed, and a C
compiler. It's probably faster than L<LWP>.
...
addresses), L<LWP::UserAgent::Determined> and
L<LWP::UserAgent::ExponentialBackoff> (user agents retrying a few
times in case of transient errors) ...

L<LWP::ParallelUA> is an implementation on top of L<LWP> to allow
parallel fetching. Unfortunately this module does not work anymore
with recent L<LWP> installations.
...
L<Web::Scraper> is a web scraping toolkit using HTML/CSS selectors or
XPath expressions.

L<Scrappy> is a powerful web harvester and spider.
...
L<Starman> - a high-performance preforking PSGI/Plack web server.

L<Monoceros> - a PSGI/Plack server with event driven connection
manager, preforking workers, inherited from Starman.
...
L<Plack> - PSGI toolkit, may also be used to create web sites.
...
L<URI::Query> handles just the querystring part.
...
size) is L<Apache::Dynagzip> (which is a
L<Apache::Filter>-implementing filter). More light-weight solutions
are L<Apache::Compress> (also in conjunction with L<Apache::Filter>)
and L<Apache::GzipChain> (in conjunction with L<Apache::OutputChain>).
All three can handle static files. L<Apache::Dynagzip> has recipes how
to work with L<CGI> or L<Apache::Registry> scripts).
