...
die Schemasprachen XML-Schema, RelaxNG und Kwalify verglichen werden.
Hier wird die Mächtigkeit der Schemasprache und die
E<quot>KostenE<quot> (Erstellung? Wartung? Verständnis? Lesbarkeit?)
gegenübergestellt. Hier sieht man, dass XML::Schema vergleichsweise
mächtig ist, dafür aber mit hohen E<quot>KostenE<quot> einhergeht. Die
Mächtigkeit von Kwalify ist hingegen nicht so hoch, die Erstellung ist
aber relativ billig.
...
manueller Aufwand notwendig (einfach XML::Simple verwenden geht
nicht!), teilweise erheblicher (es gibt jetzt aber XML::Compile!). Bei
der Verwendung von Kwalify und damit normaler Serialisierung entfällt
das.

=head2 Typen

Die Schemasprache beherrscht grundlegende Perl-Typen:
...
Einschränkung auf Aufzählungen (enum) oder per regulären Ausdruck,
Zahlenbereiche etc.

In der Kwalify-Spezifikation ist auch E<quot>assertE<quot>
spezifiziert, mit denen weitere Bedingungen mithilfe von Programmcode
ausgedrückt werden können. Dies ist bislang nur in der
Ruby-Implementation möglich und dort auch als
E<quot>experimentellE<quot> gekennzeichnet.
